...
import { DashboardMobileMenu, DashboardNav, } from "@/features/dashboard/components/dashboard-nav";
import { DashboardSummaryProvider } from "@/features/dashboard/context/dashboard-summary-context";
...
export function DashboardShell({ title, description, breadcrumb, actions, children, layout = "default", }: Props) {
        ...
        >
          ...
          >
            <div
              className={
                isChat
                  ? "flex h-full max-h-[calc(100dvh-var(--site-header-height)-1.75rem)] flex-col border-r border-border/60 bg-transparent p-3"
          ...
          >
            ...
            <div className={isChat ? "flex min-h-0 min-w-0 flex-1 flex-col" : undefined}>{children}</div>
          </div>
        </div>
      </Container>
    </DashboardSummaryProvider>
